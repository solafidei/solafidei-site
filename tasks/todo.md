...
- [ ] Task 5: Design pass — impeccable PRODUCT.md, tokens, chip, base components — M · opus · deps: 1
- [ ] Task 6: Copy pass A — hand-authored data JSON + manifest.facts[] (nine illustrative ids incl. fitting-durations, per §6.0 — verified against the spec, not the shorter §11 gloss) — M · opus · deps: 3, 4
...
- [ ] Task 8: Shared header + contact block byte-identical across five pages (group 4) via scripts/gen-shared-blocks.mjs — deviation from spec §5 tree noted — M · sonnet · deps: 5, 7
- [ ] Task 9: site.js — WhatsApp link builder, isBuyable(), stock-state map (split out from the old combined task 8; independent of task 8, both consumed starting task 12) — S · sonnet · deps: 2, 5
