...
- [ ] Slide 10 contains the literal "from R ______ / month" and prints no monthly rand figure. Two exceptions, ruled #650 after this AC was found to contradict §6.7: `R0 upfront` stays (§6.7 mandates it, it is not a monthly figure, and the gate regex `/R ?[0-9][0-9,]*\s*\/\s*month/` cannot fire on it), and the ≈R700–R1,550/month tools range is NOT printed (#491c's "no pass-through line" is operative, and printing it would fail the gate outright). The day-30 get-out and the tools-inside-the-retainer line appear exactly as ruled.
...
### Task 21a: Price comma cell fix + Roll-Line wording fix + six-screenshot re-shoot (#646, #650)

**Description:** `.price` sets `font-family: var(--font-mono)` with `font-variant-numeric: tabular-nums` (`site.css:257-258` and `:646-647`), so the thousands separator occupies a full digit cell and every four-figure price renders as `R2 , 700` where the markup correctly says `R2,700`. Pixel review at CP6 found it on essentially every four-figure price on Home, the roller-derby hub and the PDP, so it reaches every product image T22's canvas uses; it was previously carried as a closed task's cosmetic choice and was ruled a real defect at the gate (#646). Fix the rendering only — keep tabular figures so price columns stay aligned across cards, but stop the separator claiming a digit cell (wrap the separator, or set `font-variant-numeric`/`font-feature-settings` so the comma is proportional while the digits are not). Do not touch the markup, which is already correct, and do not change any price **value**: `R700.01` on Riedell Superball Toestop Long (SKU 9571) is Mel's genuine live store price, preserved under #549/#563 and asserted by gate G7. Then re-run `scripts/shoot-mels-demo.mjs` against `npm run start` so the six screenshots T22 consumes carry clean prices. **Second, folded in at ruling #650:** the demo asserts an exclusivity claim the benchmark report does not support. Six places say Mel's is "Roll-Line's only South African dealer" / "the sole South African Roll-Line dealer": `demo/data/draft-copy.json:20` (`heroNote`) and `:56` (`trustRow.dealerCardLabel`), `demo/data/manifest.json:29` (the `roll-line-dealer` fact) and `:725` (the `roll-line-listing.webp` capture note), and `demo/index.html:56,182` — **which are generated, so edit `draft-copy.json` and re-run the generator, never the HTML** (`scripts/gen-home.mjs:225` renders `heroNote`, `:427` renders `dealerCardLabel`). The report supports no such claim: L12 and L40 say only that Roll-Line's dealer database, queried live, lists Mel's; L41 says dealer status is "genuinely scarce" *because Roll-Line sells to no private buyers*; grepping the report for "only/sole South African dealer" returns nothing. The task-21 deck already declines the claim and uses L41's scarcity framing, so slide 4 currently taps through to an artefact claiming more than the deck will, while slide 12 promises every number is sourced or marked proposed — and Mel can disprove this one. Replace all six with L41's framing (listing + "Roll-Line sells to no private buyers, so dealer status is genuinely scarce"); the short card label becomes the report's own verbatim "Authorised Roll-Line dealer". Do not invent a replacement superlative. **Sequencing: the wording fix must land before the re-shoot** — `heroNote` renders in the Home hero, so it is inside `01-home.png`, and re-shooting first wastes the capture. Read the T20 evidence note's four negative controls before altering capture order — the untimed `await img.decode()` at `shoot-mels-demo.mjs:140` is unguarded and is only safe because the eager flip precedes it, so a reorder turns a failing assertion into a hang.
...
- [ ] No artefact under `public/decks/mels-skate-shop/` claims Mel's is Roll-Line's only/sole South African dealer; all six sites carry L41's scarcity framing instead, `index.html` is regenerated rather than hand-edited, and no replacement superlative is introduced.
...
- [ ] `cd ~/solafidei-site && grep -rn "only South African\|sole South African\|only Roll-Line" public/decks/mels-skate-shop/ scripts/; echo "expect no matches"`
- [ ] `cd ~/solafidei-site && npm run lint && git status --short`

**Dependencies:** 21 · **Model:** sonnet · **Estimated scope:** S · **Issue:** #49
**Files likely touched:** `public/decks/mels-skate-shop/demo/assets/site.css`, `public/decks/mels-skate-shop/demo/data/draft-copy.json`, `public/decks/mels-skate-shop/demo/data/manifest.json`, `public/decks/mels-skate-shop/demo/index.html` (generated), `public/decks/mels-skate-shop/.shots/*.png`
**Spec refs:** §4, §9, §10
