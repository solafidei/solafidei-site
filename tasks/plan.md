...
- [ ] `cd ~/solafidei-site && for id in fit-guarantee pjn-instalments fitting-prices fitting-durations cancellation-policy heat-mould-price mail-in-heat-mould aura-size-run demo-buy-button; do sed -i "s#</body>#<span data-illustrative=\"$id\">x</span></body>#" public/decks/mels-skate-shop/demo/index.html; done && node scripts/verify-mels-demo.mjs --only=11; echo "expect exit 0 with all nine injected: $?"; sed -i '/data-illustrative="demo-buy-button"/d' public/decks/mels-skate-shop/demo/index.html && node scripts/verify-mels-demo.mjs --only=11; echo "expect non-zero naming missing demo-buy-button: $?"; git checkout -- public/decks/mels-skate-shop/demo/index.html`
- [ ] `cd ~/solafidei-site && npm run lint`
