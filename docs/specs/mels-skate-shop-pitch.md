...
3. **Roller Derby on Mel's site is category 120, 8 SKUs** (Riedell R3 sets, Chaya Ruby, Sure-Grip Rebel/Phoenix/Avanti, Riedell R3 Derby, a gift voucher). The hub grid shows those (minus the voucher) plus derby-relevant real SKUs from Sets (27), Wheels & Bearings (38) and Toe Stops (17), grouped by decision cards. Store API sample trap: `is_in_stock` was true for every sampled SKU; the in-stock toggle reads `is_purchasable && is_in_stock` and `stock_availability.text`.
